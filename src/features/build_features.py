...
#Export du dataframe 
df.to_csv('data/processed/weatherAUS_processed_data.csv')

# Séparation des variables explicatives (data) et de la variable cible (target)
data = df.drop('RainTomorrow',axis = 1) #(142 190, 17)
target = df['RainTomorrow'] #(142 190,)

# Séparation des données en jeu d'entrainement et de test
X_train, X_test, y_train, y_test = train_test_split(data, target, test_size = 0.2, random_state=42)

#Normalisation des données explicatives sur X_Train et X_test calculée à partir des données de X_train
scaler = MinMaxScaler()
...
X_rus.to_csv('data/processed/X_rus.csv')
y_rus.to_csv('data/processed/y_rus.csv')

# Undersampling Cluster Centroids
cc = ClusterCentroids()
X_cc, y_cc = cc.fit_resample(X_train, y_train)
X_cc.to_csv('data/processed/X_cc.csv')
y_cc.to_csv('data/processed/y_cc.csv')

#########  Oversampling ###############
# Ramdom Oversampling
ros = RandomOverSampler()
X_ros, y_ros = ros.fit_resample(X_train, y_train)
X_ros.to_csv('data/processed/X_ros.csv')
y_ros.to_csv('data/processed/y_ros.csv')

# Oversampling  SMOTE
smo = SMOTE()
X_smo, y_smo = smo.fit_resample(X_train, y_train)
X_smo.to_csv('data/processed/X_smo.csv')
y_smo.to_csv('data/processed/y_smo.csv')